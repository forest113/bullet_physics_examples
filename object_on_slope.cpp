...
int main(int argc, char** argv)
{
	...
	//the ground is a cube of side 100 at position y = -56, and is rotated around x axis by 30 deg.
	//the sphere will hit it at around y = -5.
	//After collision, the sphere will roll down the slope, developing angular velocity in the x direction
	//(as plane is rotated in x dir.)
	{
		...
		groundTransform.setRotation(btQuaternion(btVector3(1, 0, 0), 30));
	...
	printf("the ground is a cube of side 100 at position y = -56, and is rotated around x axis by 30 deg. /n the sphere will hit it at around y = -5.\nAfter collision, the sphere will roll down the slope, developing angular velocity in the x direction(as plane is rotated in x dir.)\n\n ");
	...
	for (i = 0; i < 150; i++)
	{
		...
		for (int j = dynamicsWorld->getNumCollisionObjects() - 1; j >= 0; j--)
		{
			...
			{   //detect overlaps
				...
				for (int i = 0; i < numManifolds; i++)
				{
					...
					for (int j = 0; j < numContacts; j++)
					{
						...
						if (pt.getDistance() <= 0.f)
						{
							...
							const float& i = pt.getAppliedImpulse();
							const btVector3& normalOnB = pt.m_normalWorldOnB;
							printf("******collision at  = %f,%f,%f maniflods=%d, contact points= %d*********\n impulse = %f\n", float(ptA.getX()), float(ptA.getY()), float(ptA.getZ()), numManifolds, numContacts,i);
							//printf("******collision point on sphere  = %f,%f,%f contact point=%d*********\n", float(ptB.getX()), float(ptB.getY()), float(ptB.getZ()), j);
						}
					}
				}

				//print location, vel and acceleration
				btVector3 force = body->getAngularVelocity();
				btScalar fric = body->getRollingFriction();
				...
				printf("Angular vel = x:%f,y:%f,z:%f, rolling fric:%f\n\n", float(force.getX()), float(force.getY()), float(force.getZ()),fric);
			}
		}
	}
